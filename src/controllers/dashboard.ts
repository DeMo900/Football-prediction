...
async function checkReward(req: Request, res: Response) {
  try {
    const { id } = jwt.verify(req.cookies.jwt, process.env.JWT_SECRET!) as {
      id: string;
    };
    const user = await pool.query(
      "SELECT last_claim FROM users WHERE id = $1",
      [id],
    );
    if (user.rowCount === 0)
...
async function claimReward(req: Request, res: Response) {
  try {
    ...
    const deletedCount = await redis.del(`rewardForUser:${id}`);
    if (deletedCount === 0)
      return res.status(404).json({ msg: "no reward available" });
    const updatedUser = await pool.query(
      "UPDATE users SET coins = coins + 10, last_claim = $1 WHERE id = $2 RETURNING coins",
      [Date.now(), id],
    );
    ...
    return res
      .status(200)
      .json({ message: "reward claimed", coins: updatedUser.rows[0].coins });
  } catch (err) {
    console.log(err);
    return res.status(500).json({ msg: "internal server error" });
  }
...
async function getUser(req: Request, res: Response) {
  try {
    ...
    const user = await pool.query(
      "SELECT username,coins FROM users WHERE id = $1",
      [id],
    );
    if (user.rowCount === 0)
      return res.status(404).json({ msg: "user not found" });
